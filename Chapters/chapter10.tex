...
There are many other commonly used schemes summarized in   {Paterson et al., 2014}  \nocite{paterson14}  who also compiled all the known 
...
$^a$SBG is submarine basaltic glass.  
\end{tabular}

Which paleointensity statistics are the most useful?  What are the advantages and drawbacks of the two sets of selection criteria used here?   
...
and download the data as in Problem 10.1.  Make a new project directory and copy the downloaded file into it.   Unpack the data file with the command {\bf download\_magic.py -f upload.txt}, or from within a jupyter notebook using the initial '!' as usual.   


b) Find your \href{http://earthref.org/PmagPy/#command_line}{command line} and change directories into your new project directory by typing {\it cd MY\_DIRECTORY}, where you type the directory name instead of MY\_DIRECTORY. Then type  {\bf biplot\_magic.py}.    This will give you a list of available options for plotting measurement data.   In this case they are 'LT-AF-Z', 'LT-AF-I', 'LT-IRM', 'LP-X', which are `method codes' in MagIC.   On the \href{http://earthref.org/MAGIC} {http://earthref.org/MAGIC}  website, follow the link to ``Method Codes''.   Examine the available options under  ``Lab Protocol'' and ``Lab Treatment''  and find the option that describes   these.   
