...
a) Calculate the expected direction for the Miocene (make a guess at  the numerical age from the information supplied in Chapter 15) at a locality in the Mojave Desert (latitude of 34.5$^{\circ}$N and longitude  of 117.5$^{\circ}$W).   Use the program {\bf apwp.py}.    Assume that the Mojave  was on the North American Plate.  




b) Go to the MagIC search interface  at: \url{earthref.org/MAGIC/search}.    Click on the search icon and use the `Location Name contains' filter in the Location Filters.   Find the studies from these locations and download the datasets:     Western Vasquez, Eastern Vasquez and Central Vasquez, Mojave Desert Volcanics,  Pink Canyon,  Hector Formation,   and Fort Irwin.  Some of these are combined in the same studies, so you should have four datasets to play with.  


Make an equal area  plot of all these results  and calculate the  average direction and paleomagnetic pole for these Miocene rocks.   
...
Go to the GPMDB website at: ngu.no/geodynamics/gpmdb/
...
Put the pole latitudes and longitudes into a file and use the program {\bf pt\_rot.py} to rotate these to North American coordinates for 100 Ma.  Look up the poles of rotation in Appendix~\ref{app:polerot} for first rotating Europe to fixed African coordinates, then African to North American coordinates (you will have to change the 
 sign of $\Omega$ for the latter).     Save your rotated poles in a file using the '-F' option (creates a MagIC formatted pmag\_results file)  and plot these using the program {\bf vgpmap\_magic.py}.       Compare your rotated pole with the synthetic pole of Besse and Courtillot (2002).  You can find what this is by using the program {\bf apwp.py}.   
