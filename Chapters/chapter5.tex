...
{\bf Problem 2} [From Jeff Gee]


In this problem, we will begin to use some real data.  The data files used with this book are part of the {\bf PmagPy} distribution, which you should have already downloaded and installed.  [See Preface for instructions.]  

The file {\it hysteresis.txt) in the Chapter\_5 directory contains data for a single hysteresis loop. Note that the units  are as measured: H (Oe), moment (emu) and it is fine to leave them in these units.
 
a) Read the data into a {\bf Pandas} DataFrame.  Determine the high field slope at $|H| > 4000$ Oe. Typically one calculates separate slopes for the +H data and -H data and averages these. A general least squares polynomial fit (numpy.polyfit) should do the trick.
