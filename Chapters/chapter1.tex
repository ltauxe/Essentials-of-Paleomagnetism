...
Before you do any of the problems in this book, you will need to know your way around a computer.  Therefore, you are strongly encouraged to consult the  \href{http://earthref.org/PmagPy/}{PmagPy Cookbook website}. Follow the instructions in Chapter 1 (Installing Python and PmagPy),  Chapter 4 (Survival computer skills), Chapter 7 (Intro to Python Programming) and Chapter 8 (Jupyter Notebooks).     Using your `survival skills', make a directory called {\it Problems} (or something similar) on your computer in which you will do all the problems for this book.  Make a subdirectory for Chapter01. 

After installing Python and the  {\bf PmagPy} package, find the directory called `Essentials\_Examples'  in the data\_files folder and copy it to your Problems directory.  Find your command line (for help see the  \href{http://earthref.org/PmagPy/cookbook/#command_line}{PmagPy cookbook}) and navigate to the Problems directory.   Now type on the command line:  
\begin{verbatim}
$ jupyter notebook   
\end{verbatim}
and open the  file `essentials\_ps\_1\_template.ipynb' in the `Notebooks' directory of the data\_files folder.   Under the 'File' menu, select `Make a Copy' and rename your notebook `MYNAME\_ps\_1.ipynb', where MYNAME is your name (with no spaces), e.g., LimaTango.  Save this file in your Problems folder.   
...
a) In a jupyter notebook, write python functions to convert induction, moment and magnetic field quantities in cgs units to SI units.   Use the conversion factors in Table~\ref{tab:units}.    Use your function  to convert the following from cgs to SI: 
