...
\begin{figure}[htb]
 \centering   \includegraphics[width=10cm]{EPSFiles/wire.eps} 
...
\noindent  So, now we know the units of $\m$:   Am$^2$.    In nature, magnetic moments are carried by magnetic minerals the most common of which are magnetite and hematite (see Chapter 6 for details). 
   ...
   \begin{figure}[htb]
\centering  \includegraphics[width=14 cm]{EPSfiles/moment.eps}
...
magnetic field:  the magnetic induction $\B$.  
        ...
        \begin{figure}[htb]
\centering  \includegraphics[width=14 cm]{EPSfiles/barmagnetfield.eps}
...
$B$ are the magnitudes of $\m$ and $\B$, respectively (see Section~\ref{app:vecmult} in the appendix for review of vector multiplication). Magnetic energy has units of joules and is at a minimum when $\m$ is aligned with $\B$.
 ...
 \begin{figure}[htb]
 \centering \includegraphics[width=12 cm]{EPSfiles/compass.eps}
...
\begin{figure}[htb]
 \centering \includegraphics{EPSfiles/pp.eps}
...
\begin{figure}[htb]
 \centering \includegraphics[width=12 cm]{EPSfiles/divergence.eps}
...
\begin{figure}[h!tb]
 \centering \includegraphics[width=6 cm]{EPSfiles/mH.eps}
\caption
...
\begin{figure}[h!]
 \centering \includegraphics[width=11.5 cm]{EPSfiles/discdynamo.eps}
...
Before you do any of the problems in this book, you will need to know your way around a computer.  Therefore, you are strongly encouraged to consult the  \href{http://earthref.org/PmagPy/}{PmagPy Cookbook} website. Follow the instructions for installing Python and PmagPy,  read the chapter on survival computer skills,  and look through the introductory chapter on  Python Programming as well as the one on Jupyter notebooks.   After you install PmagPy, be sure to follow the instructions on how to move the data\_files directory to somewhere on your computer, say, the Desktop, or your home directory.     Using your `survival skills', make a new directory called {\it Problems} (or Homework or something similar) on your computer in which you will do all the problems for this book.  Make a subdirectory for Chapter01 to work the problems in this section. 

Find the directory called `Essentials\_Examples'  in the data\_files directory and copy it to your Problems directory.  Find your command line (for help see the  \href{http://earthref.org/PmagPy/cookbook/#command_line}{PmagPy cookbook}) and navigate to the Problems directory.   Now type on the command line:  
\begin{verbatim}
$ jupyter notebook   
\end{verbatim}
First open the file essentials\_ch\_0.ipynb notebook  in the `Notebooks' directory of the data\_files folder.  which is a version of the Python Programming chapter from the cookbook, written to work with jupyter notebooks.  This will give you many tools useful for solving the problems in this book.  Notebooks are constructed as a series of `cells' which can be text or code among other things.  To view the `source' of a text cell, just click on it.  To render it, click on the `run' button (sideways triangle on the toolbar).  Text can be written in the style of LaTex to make nice looking equations, so you might want to familiarize yourself with that syntax.  To run the code in a code cell, click on the cell and then the `run' button.   Anything after a pound sign (\#) in a code block will be ignored, so this is how we comment code.  Run the code block with the comment `\# My first program.  Then make a new code block and have it print your favorite pithy phrase. 


Now open the  file `essentials\_ch\_1\_template.ipynb' Under the 'File' menu, select `Make a Copy' and rename your notebook  with your name and homework number in the title.  Save this file in your Problems/Chapter01 folder.   Modify this template to solve the following problems.  
...
a) In your jupyter notebook, write python functions to convert induction, moment and magnetic field quantities in cgs units to SI units.   Use the conversion factors in Table~\ref{tab:units}.    Use your function  to convert the following from cgs to SI: 
...
%\centering \epsffile{EPSfiles/dipole.eps}
 \centering \includegraphics[width=6 cm]{EPSfiles/dipole.eps}
\caption{A magnetic dipole source (${\bf m}$) located 3480 km from the center of the Earth, below the point of observation at 45$^{\circ}$ latitude.  The average radius of the Earth is 6370 km.  The field at the point of observation is directed  downward (toward the center) with a magnitude of 10 $\mu$T.  }
\label{fig:dipole}
\end{figure}
...
Knowing that $B = \mu_o H$, work out the fundamental units of $\mu_o$ in SI units.   Prepare your answer in a markdown cell in your jupyter notebook.  
